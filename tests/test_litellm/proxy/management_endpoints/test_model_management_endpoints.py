...
class TestStrategyRouterWriteValidation:
    ...
    def test_settings_beside_the_config_rejected_on_a_patch_of_a_stored_router(self):
        """The patch carries only the stray key, so scope has to come from the stored config:
        the stored model is encrypted at rest and cannot be classified here."""
        from litellm.proxy.management_endpoints.model_management_endpoints import (
            _strategy_router_write_violation,
        )
        from litellm.types.router import updateLiteLLMParams
