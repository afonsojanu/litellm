...
def carries_complexity_router_settings(litellm_params: Mapping[str, object]) -> bool:
    """Whether these params configure a complexity router, and so are judged on its key set.
    ...
    on an s3_vectors vector store.
    """
    return litellm_params.get("complexity_router_config") is not None or (
        classify_strategy_router_model(str(litellm_params.get("model") or "")) == "complexity"
    )
